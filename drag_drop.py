...
class MainWidget(QMainWindow):
    def __init__(self):
        ...
        self.file_display_label.move(int(container_dimensions[0]/2-self.file_display_label.rect().width()/2),int(widget_buffer[1]-text_field_size[1]/2-25))
        self.file_display_label.setText("File To Duplicate (Drag File):")

        text_field_size = (150,30)

        self.added_text = QTextEdit(self)
        self.added_text.move(widget_buffer[0],int(container_dimensions[1]/2-text_field_size[1]/2))
        self.added_text.resize(text_field_size[0],text_field_size[1])
        self.added_text_label = QLabel('Center', self)
        self.added_text_label.resize(120,30)
        self.added_text_label.move(self.added_text.pos().x(),int(container_dimensions[1]/2-text_field_size[1]/2-25))
        self.added_text_label.setText("Text To Add:")

        text_field_size = (60,30)

        self.starting_number_text = QTextEdit(self)
        self.starting_number_text.move(container_dimensions[0]/2-20,int(container_dimensions[1]/2-text_field_size[1]/2))
        self.starting_number_text.resize(text_field_size[0],text_field_size[1])
        self.starting_number_text.setPlainText("1")
        self.starting_number_text.textChanged.connect(self.onAmountChange)

        self.starting_number_label = QLabel('Center', self)
        self.starting_number_label.resize(120,30)
        self.starting_number_label.move(self.starting_number_text.pos().x(),int(container_dimensions[1]/2-text_field_size[1]/2-25))
        ...
        self.copy_amount_label.move(self.copy_amount_text.pos().x(),int(container_dimensions[1]/2-text_field_size[1]/2-25))
        ...
        self.duplicate_button.move(int(container_dimensions[0]/2-self.duplicate_button.rect().width()/2),int(container_dimensions[1]-widget_buffer[1]-self.duplicate_button.rect().height()/2))
